...
- Shared SVG icons now cover the visible world, story details, custom world, new character, narrator, progress error, reader, song, permission, low-credit and library flows. Remaining inline SVG helpers are limited to Home/account internals and should be handled in the next icon pass.
...
- Home appearance choices now visibly affect the Home CTA/background grade and persist locally under `nicky-appearance-v1`; account sync and true app-wide propagation remain pending.
...
- Wire `Vymysli námět`, `Rozvinout`, `Prostudovat` and location permission to production-safe handlers; the preview currently demonstrates UI/navigation only.
