...
function buildSystemPrompt(language: "cs" | "en"): string {
  if (language === "en") {
    return [
      ...
      "- `heroDescription` is the SINGLE SOURCE OF TRUTH for character appearance — it MUST NOT change between scenes.",
      "- For EVERY character include: hair color, eye color, approximate height, clothing (specific color and type), whether adult males have a beard or not, and any key features (bow, freckles, glasses, etc.).",
      "- Every `imagePrompt` MUST copy these EXACT visual traits verbatim for EACH character who appears in the scene.",
      "- Appearance NEVER changes between scenes: no beard in scene 1 = no beard in scenes 2-8; same clothing color throughout.",
      "- If reference photos are attached, descriptions MUST match the photos exactly.",
  ...
  return [
    ...
    "- `heroDescription` je JEDINÝ ZDROJ PRAVDY o vzhledu postav — NESMÍ se měnit mezi scénami.",
    "- `heroDescription` piš ANGLICKY (je předáno image generátoru — žádné věkové údaje, pouze vzhled).",
    "- U KAŽDÉ postavy uveď: barvu vlasů, barvu očí, přibližnou výšku, oblečení (konkrétní barva a typ), zda dospělý muž má nebo nemá vousy, a klíčové rysy (luk, pihy, brýle apod.).",
    "- Každý `imagePrompt` MUSÍ doslova zkopírovat tyto přesné vizuální rysy pro KAŽDOU postavu, která ve scéně vystupuje.",
    "- Vzhled se NIKDY nemění mezi scénami: bez vousů ve scéně 1 = bez vousů ve scénách 2–8; stejná barva oblečení po celý příběh.",
    "- Pokud jsou přiloženy referenční fotografie, popisy MUSÍ odpovídat fotografiím přesně.",
