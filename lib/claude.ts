...
function buildSystemPrompt(language: string): string {
  ...
  if (langName) {
    return [
      ...
      '  WEATHER: "waves" (sea/ocean lapping or crashing) · "thunder" (thunder/lightning striking) · "wind_gust" (a sudden gust of wind) · "rain" (rain falling) · "snow_crunch" (footsteps crunching in snow)',
      '  ANIMALS (the animal itself makes its sound in THIS scene): "cow" (moo) · "pig" (oink) · "chicken" (cluck) · "sheep" (bleat) · "horse" (neigh) · "duck" (quack) · "dog" (bark) · "cat" (meow) · "frog" (croak) · "owl" (hoot) · "rooster" (crow) · "bee" (buzzing)',
      ...
      "SOUND MUST MATCH THE PICTURE: whenever you DO set `sfx`, the imagePrompt for that same scene MUST visibly show that exact sound happening — the listener hears it AND sees it. An animal `sfx` needs the animal drawn mid-sound (mouth open barking/mooing/quacking, an expressive sound-making pose), an instrument `sfx` needs a character actively playing it (bow on the violin strings, hands on the piano keys, fingers on the guitar strings), an object `sfx` needs the object shown mid-action (the bell mid-ring with motion lines, the key turning in the lock, the page mid-turn), and a weather `sfx` needs that weather visibly happening in the scene (rain falling, lightning flashing, leaves blown sideways in the wind). Never set an `sfx` whose source is off-screen or already finished by the time of the picture.",
      "",
      "═══ OUTPUT ═══",
  ...
  return [
    ...
    '  POČASÍ: "waves" (moře naráží/šplouchá) · "thunder" (hrom/blesk udeří) · "wind_gust" (náhlý poryv větru) · "rain" (padá déšť) · "snow_crunch" (kroky křupou ve sněhu)',
    '  ZVÍŘATA (zvíře se v TÉTO scéně samo ozve): "cow" (bučení) · "pig" (chrochtání) · "chicken" (kdákání) · "sheep" (bečení) · "horse" (řehtání) · "duck" (kvákání) · "dog" (štěkání) · "cat" (mňoukání) · "frog" (kuňkání) · "owl" (houkání) · "rooster" (kokrhání) · "bee" (bzučení)',
    ...
    "ZVUK MUSÍ SEDĚT S OBRÁZKEM: kdykoli `sfx` NASTAVÍŠ, imagePrompt téže scény MUSÍ ten přesný zvuk viditelně ukazovat — posluchač ho slyší A VIDÍ. Zvíře u `sfx` potřebuje být nakreslené PŘI zvuku (otevřená tlama při štěkání/bučení/kvákání, výrazná póza), nástroj potřebuje postavu AKTIVNĚ hrající (smyčec na strunách houslí, prsty na klávesách klavíru, prsty na strunách kytary), předmět potřebuje být zachycený PŘI akci (zvoneček uprostřed zvonění s pohybovými čarami, klíč otáčející se v zámku, stránka uprostřed otáčení), a počasí u `sfx` potřebuje to počasí viditelně probíhat ve scéně (padající déšť, blesk, vítr čechrající listí do strany). Nikdy nenastavuj `sfx`, jehož zdroj je mimo záběr nebo už v obrázku dávno doznělý.",
    "",
    "═══ VÝSTUP ═══",
