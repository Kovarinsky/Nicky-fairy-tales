...
function buildSystemPrompt(language: string): string {
  ...
  if (langName) {
    return [
      ...
      "",
      "═══ SOUND EFFECT (sfx) ═══",
      "Optional `sfx` field on a scene — a ONE-SHOT sound effect for something the NARRATION OF THAT SCENE explicitly describes happening RIGHT THEN, on top of the ambient soundscape. Only add it when the text truly calls for it — most scenes have none. Never force one just to fill the field. Pick the single BEST match from this list (grouped by category, choose the exact key on the left):",
  ...
  return [
    ...
    "",
    "═══ ZVUKOVÝ EFEKT (sfx) ═══",
    "Volitelné pole `sfx` u scény — JEDNORÁZOVÝ zvukový efekt navíc k ambientnímu soundscape, pro něco, co DĚJ TÉTO KONKRÉTNÍ scény výslovně popisuje PRÁVĚ TEĎ. Přidej ho, jen když to text opravdu vyžaduje — většina scén ho mít nebude, nikdy ho nevynucuj. Vyber JEDEN nejlepší klíč z tohoto seznamu (anglicky, přesně jak je napsáno):",
