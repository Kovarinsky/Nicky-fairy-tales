...
function buildSystemPrompt(language: string): string {
  ...
  if (langName) {
    return [
      ...
      "- INVENTED CHARACTERS MUST LOOK DELIBERATELY DIFFERENT from the canonical family (CRITICAL — the illustrator only has reference portraits of the FAMILY, so any invented person you describe vaguely, or describe similarly to a family member, comes out looking like a copy of that family member): every invented HUMAN gets a DIFFERENT hair color AND a different hair style from every canonical character in this story, plus at least two more clearly contrasting features (build, face shape, skin tone, freckles, glasses, a hat, a beard for adults, distinctly different clothing colors). If an invented child is the same age and gender as a canonical child hero, make the contrast even louder — e.g. dark curly hair and olive skin against straight light-blond hair and fair skin. Never describe an invented character with the same hair color as a hero, and never leave their hair, face or clothing to the illustrator's imagination.",
      ...
      "- Do NOT default to the same handful of go-to devices: magical sparkling dust that fixes everything, a magic coin/amulet/pendant that grants wishes, a moral that boils down to 'do it with a kind/good heart', a house/toy/object that turns out to secretly have a heart or feelings, OR a small lost/scared magical creature (a star, firefly, cloud, baby animal...) that the heroes simply find and guide back home — that last one especially is a heavily overused default PREMISE, not just a trope detail, and must not become the go-to opening idea story after story. These are fine ONLY when the user's own topic, world, or theme specifically calls for that kind of magic — otherwise resolve the plot through a DIFFERENT engine: a mystery solved with clues and deduction, a skill practiced and improved, a misunderstanding untangled through honest conversation, a real-world obstacle (weather, distance, something broken, a disagreement) overcome through effort/cleverness/teamwork, a competition, an exploration or discovery.",
      ...
      "- VARY THE PREMISE/MOTIF ACROSS STORIES: when the user hasn't given a specific topic, pick the STARTING SITUATION from a wide, genuinely different pool each time — e.g. a mystery/clue to solve, a competition or contest, a journey/expedition somewhere new, a skill to learn or perform, a misunderstanding between friends to untangle, a search for something (not necessarily a scared lost creature), a celebration or event to prepare/save, a real-world obstacle to overcome, an unexpected visitor, a rescue, an invention or building project, a rule or game to figure out. Do not let consecutive stories for the same reader converge on the same premise shape even if the surface details (world, characters) differ.",
      ...
      "- DEFAULT TO SHARED INTERACTION: unless the plot explicitly requires someone to watch a distant object, stand guard, leave, hide, sleep, or be alone, describe every visible person and animal engaging with the shared action — gaze, expression and body oriented toward another character or the jointly relevant object/event. Never leave a pet or side character randomly staring out of frame.",
      ...
      "CUSTOM HERO SFX (OPTIONAL, MAXIMUM TWO CUES IN THE WHOLE STORY): only when a central, distinctive sound cannot be represented closely by the library key, add `customPrompt` and `customDurationSec` (0.5–6) to that cue. The `effect` remains mandatory as a fallback. customPrompt describes ONE isolated audible event in precise English, including material, motion, intensity and environment; no music, speech or ambience. Never use custom SFX for ordinary footsteps, wind, birds, laughter, bells or other sounds already covered by the library. Zero custom cues is correct; never exceed two.",
  ...
  return [
    ...
    "- VYMYŠLENÉ POSTAVY MUSÍ VYPADAT ZÁMĚRNĚ JINAK než kanonická rodina (KRITICKÉ — ilustrátor má referenční portréty JEN rodiny, takže každá vymyšlená osoba, kterou popíšeš vágně nebo podobně jako člena rodiny, vyjde jako jeho kopie): každý vymyšlený ČLOVĚK dostane JINOU barvu vlasů A jiný střih než kterákoli kanonická postava v této pohádce, plus aspoň dva další jasně kontrastní znaky (postava, tvar tváře, tón pleti, pihy, brýle, čepice, u dospělých vousy, výrazně jiné barvy oblečení). Když je vymyšlené dítě stejného věku a pohlaví jako kanonický dětský hrdina, kontrast ještě zesil — např. tmavé kudrnaté vlasy a olivová pleť proti rovným světle blond vlasům a světlé pleti. Nikdy nedávej vymyšlené postavě stejnou barvu vlasů jako hrdinovi a nikdy nenechávej její vlasy, tvář ani oblečení na fantazii ilustrátora.",
    ...
    "- Nesahej defaultně pořád po stejné hrstce berliček: kouzelný třpytivý prášek, co vyřeší cokoli, kouzelná mince/amulet/přívěsek plnící přání, ponaučení, co se scvrkne na „stačí to dělat s dobrým srdíčkem“, domeček/hračka/předmět, co se nakonec ukáže mít srdce a city, NEBO malé ztracené/vystrašené kouzelné stvoření (hvězdička, světluška, obláček, mládě zvířete...), které hrdinové prostě najdou a doprovodí zpátky domů — tohle poslední je obzvlášť omletá VÝCHOZÍ ZÁPLETKA, ne jen detail, a nesmí se stát pohádku za pohádkou tím prvním nápadem, po kterém saháš. Tohle je v pořádku JEN když si to vyloženě žádá uživatelovo vlastní zadání, svět nebo téma — jinak vyřeš zápletku JINÝM motorem: záhada řešená stopami a úsudkem, dovednost, kterou hrdina trénuje a zlepšuje, nedorozumění rozmotané upřímným rozhovorem, reálná překážka (počasí, vzdálenost, něco rozbitého, hádka) překonaná snahou/důvtipem/týmovou prací, soutěž, průzkum nebo objev.",
    ...
    "- STŘÍDEJ NÁMĚT/ZÁPLETKU NAPŘÍČ POHÁDKAMI: když uživatel nezadal konkrétní námět, zvol VÝCHOZÍ SITUACI ze široké, opravdu různorodé nabídky — např. záhada/stopa k rozluštění, soutěž nebo klání, výprava/cesta na nové místo, dovednost, kterou je třeba se naučit nebo předvést, nedorozumění mezi kamarády k rozmotání, hledání něčeho (ne nutně vystrašeného ztraceného stvoření), oslava nebo akce k přípravě/záchraně, reálná překážka k překonání, nečekaná návštěva, záchrana, vynález nebo stavební projekt, pravidlo nebo hra, na kterou je třeba přijít. Nedovol, aby pohádky za sebou pro stejného posluchače sklouzly ke stejnému TVARU zápletky, i když se povrchové detaily (svět, postavy) liší.",
    ...
    "- VÝCHOZÍ JE SPOLEČNÁ INTERAKCE: pokud děj výslovně nevyžaduje sledování vzdáleného cíle, hlídání, odchod, schování, spánek nebo samotu, popiš každou viditelnou postavu i zvíře zapojené do společné akce — pohled, výraz i tělo míří k jiné postavě nebo ke společnému předmětu/události. Pes ani vedlejší postava nikdy bezdůvodně nekoukají mimo obraz.",
    ...
    "ZAKÁZKOVÝ HERO SFX (VOLITELNÉ, NEJVÝŠE DVA CUE V CELÉ POHÁDCE): jen pokud ústřední charakteristický zvuk nejde dobře zastoupit knihovním klíčem, přidej k cue `customPrompt` a `customDurationSec` (0.5–6). Pole `effect` zůstává povinnou zálohou. customPrompt anglicky přesně popíše JEDINOU izolovanou slyšitelnou událost včetně materiálu, pohybu, intenzity a prostředí; bez hudby, řeči a ambientu. Nepoužívej zakázkový zvuk pro kroky, vítr, ptáky, smích, zvonky ani nic, co knihovna už pokrývá. Nula zakázkových cue je správně; nikdy nepřekroč dva.",
...
function normalizeSceneSoundCues(scene: Scene): Scene {
  ...
  const cues = raw
    ...
    .map(c => ({
      ...
      ...(typeof c.customPrompt === "string" && c.customPrompt.trim().length >= 12
        ? { customPrompt: c.customPrompt.replace(/\s+/g, " ").trim().slice(0, 450), customDurationSec: Math.max(0.5, Math.min(6, Number(c.customDurationSec) || 2.5)) }
        : {}),
    }))
    .sort((a, b) => a.at - b.at)
    .slice(0, 3);
...
const TOPIC_SPARKS_CS = [
  ...
  "oslava/svátek, který se pokazí", "vynález nebo stavba, co musí zafungovat", "kouzelný předmět s vlastní vůlí",
...
const TOPIC_SPARKS_EN = [
  ...
  "a celebration that goes wrong", "an invention or building project that has to work", "a magical object with a mind of its own",
