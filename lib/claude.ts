...
function buildSystemPrompt(language: "cs" | "en"): string {
  if (language === "en") {
    return [
      ...
      "- WEATHER & SEASON LOGIC (MANDATORY): decide the story's season and climate from its setting. If it is NOT mild weather, EVERY character MUST get a weather-appropriate outfit via the 'Story outfits:' entry: snow/winter/high mountains → warm winter jacket, hat, scarf, gloves and snow boots; hot summer/beach → light clothes, shorts/summer dress, sandals (or swimsuits when swimming); rain → raincoats and rubber boots. Keep each character RECOGNIZABLE: reuse their signature colors in the outfit (e.g. a boy known for a white T-shirt with red stripes gets a red-and-white striped winter hat). The outfit is IDENTICAL in every scene — jackets and hats never appear or disappear between scenes. NEVER draw summer clothes in snow or winter coats on a beach.",
  ...
  return [
    ...
    "- LOGIKA POČASÍ A ROČNÍHO OBDOBÍ (POVINNÁ): urči roční období a podnebí příběhu podle prostředí. Pokud NENÍ mírné počasí, KAŽDÁ postava MUSÍ dostat odpovídající oblečení přes záznam 'Story outfits:' (anglicky): sníh/zima/velehory → teplá zimní bunda, čepice, šála, rukavice a sněhule; horké léto/pláž → lehké oblečení, kraťasy/letní šaty, sandály (při koupání plavky); déšť → pláštěnky a holínky. Postavy musí zůstat ROZPOZNATELNÉ: v převleku zachovej jejich typické barvy (např. kluk známý bílým tričkem s červenými pruhy dostane červeno-bílou pruhovanou zimní čepici). Převlek je ve VŠECH scénách IDENTICKÝ — bundy a čepice mezi scénami nemizí ani se neobjevují. NIKDY letní oblečení ve sněhu ani zimní bundy na pláži.",
