...
export default function CdPreviewPage() {
  ...
  if (screen === "catalog") return <StoryCatalogScreen backgroundImage={`${A}/svety/big/krkonose.jpg`} stories={stories} onBack={() => setScreen("world")} onPick={(s) => { setMotif(`${s.name} — nové dobrodružství Nicoláska a Váji.`); setScreen("details"); }} />;
  if (screen === "details") return <StoryDetailsStep backgroundImage={`${A}/svety/big/${world.id}.jpg`} worldName={world.name} worldImage={world.image} motif={motif} onMotifChange={setMotif} characters={selected} length={length} onLengthChange={setLength} onBackToWorld={() => setScreen("world")} onBack={() => setScreen("world")} onSubmit={() => setScreen("progress")} />;
  if (screen === "progress") return <GenerationProgressScreen step={2} subIndex={1} onCancel={() => setScreen("details")} />;
  return <StoryWorldStep backgroundImage={`${A}/svety/big/krkonose.jpg`} readyWorlds={worlds} ownWorlds={[]} selectedId={world.id} onSelect={(id) => setWorld(worlds.find((w) => w.id === id) ?? worlds[0])} onOpenCatalog={() => setScreen("catalog")} onContinue={() => setScreen("details")} />;
}
