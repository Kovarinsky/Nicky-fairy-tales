...
/**
 * Nickyho pohádky — reader / playback screen.
 * Drop /public/images/reader-<scene>.jpg illustration for the current page (portrait, 1170×2532).
...
export default function ReaderScreen({
  ...
  /** The current page's story text, split into sentences (read one at a time, word-by-word). */
  sentences: string[];
  /** Pass to control playback from outside (e.g. tied to real TTS audio). Omit to let the reader drive its own demo timer. */
  playing?: boolean;
  onPlayToggle?: () => void;
  page: number;
  pageCount: number;
      ...
      <div className={styles.caption} style={{ bottom: uiVisible ? 130 : 34 }}>
        <p className={styles.captionText}>
          {words.map((w, i) => (
            <span
              key={i}
              className={!isPlaying || i < wordIdx ? styles.wordRead : i === wordIdx ? styles.wordActive : styles.wordPending}
            >
              {w}{" "}
            </span>
          ))}
        ...
        <div className={styles.panelFade}>
          <div className={styles.controls} onClick={stop}>
            <div className={styles.transport}>
              <button className={styles.navBtn} onClick={(e) => { e.stopPropagation(); onPrevPage?.(); }} aria-label="Předchozí strana">
                <ChevronIcon dir="l" />
              </button>
              <button className={styles.playBtn} onClick={togglePlay} aria-label="Přehrát / pozastavit">
                {isPlaying ? <PauseIcon /> : <PlayIcon />}
              </button>
              <button className={styles.navBtn} onClick={(e) => { e.stopPropagation(); onNextPage?.(); }} aria-label="Další strana">
                <ChevronIcon dir="r" />
              </button>
            </div>
