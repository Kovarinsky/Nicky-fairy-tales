...
a při porušení se až 2× překreslí s popisem chyby.
...
3. **Vizuální kontrola (desatero)** — každý obrázek zkontroluje kontrolní
   model proti tomuto seznamu; při porušení až 2 opravná překreslení
   s konkrétním výčtem chyb (`lib/gemini.ts`).
